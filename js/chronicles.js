/* ═══════════════════════════════════════════════════════════
   CHRONICLES.JS — 4D LIVING ARCHIVE + 15 ARTICLES + READER
   PART 1: CANVAS ENGINES
   ═══════════════════════════════════════════════════════════ */

// ── 4D LIVING ARCHIVE CANVAS ──
(function(){
  ...
  document.getElementById('chronicles').addEventListener('mousemove', function(e){
    mx = e.clientX / W;
    my = e.clientY / H;
  });
  ...
  for(var i = 0; i < 120; i++){
    particles.push({
      ...
      hue: Math.random() < 0.5 ? 40 : Math.random() < 0.7 ? 30 : 20
    });
  }
  ...
  for(var i = 0; i < 15; i++){
    orbs.push({
      baseAngle: (i / 15) * Math.PI * 2,
      radius: W * (0.18 + (i % 3) * 0.08),
      height: H * (0.25 + (i % 5) * 0.1),
      size: 6 + (i % 3) * 3,
  ...
  function drawVault(){
    var g = ctx.createRadialGradient(W * 0.5, H * 0.42, 0, W * 0.5, H * 0.42, W * 0.7);
    g.addColorStop(0, '#0e0a18');
    g.addColorStop(0.4, '#080510');
    g.addColorStop(0.75, '#040208');
    ...
    // Spiral shelves
    for(var r = 0; r < 12; r++){
      var rr = W * (0.15 + r * 0.04);
      var ry = H * (0.2 + r * 0.05);
      ctx.strokeStyle = 'rgba(60,45,35,' + (0.25 - r * 0.015) + ')';
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.ellipse(W * 0.5 + (mx - 0.5) * 50 * (r * 0.05), ry + (my - 0.5) * 30 * (r * 0.05), rr, 8 + r * 0.5, 0, 0, Math.PI * 2);
      ctx.stroke();

      // Books on shelves
      var bc = 20 - r;
      for(var b = 0; b < bc; b++){
        var a = (b / bc) * Math.PI * 2 + t * 0.002;
        var bx = W * 0.5 + (mx - 0.5) * 50 * (r * 0.05) + Math.cos(a) * rr;
        var by = ry + (my - 0.5) * 30 * (r * 0.05) + Math.sin(a) * (8 + r * 0.5);
        var cols = ['#2a1810','#3a2215','#1a1008','#4a2a18','#352015'];
        ctx.fillStyle = cols[Math.floor(Math.random() * cols.length)];
        ctx.fillRect(bx - 2, by - 6 - Math.random() * 4, 3 + Math.random() * 2, 8 + Math.random() * 4);
      }
    }

    // Stone floor
    var fg = ctx.createRadialGradient(W * 0.5, H * 0.85, 0, W * 0.5, H * 0.85, W * 0.6);
    fg.addColorStop(0, 'rgba(30,22,18,0.6)');
    fg.addColorStop(0.5, 'rgba(20,14,10,0.45)');
    fg.addColorStop(1, 'rgba(6,3,2,0.3)');
    ctx.fillStyle = fg;
    ctx.beginPath();
    ctx.ellipse(W * 0.5 + (mx - 0.5) * 60, H * 0.82, W * 0.55, H * 0.12, 0, 0, Math.PI * 2);
    ctx.fill();

    // Floor reflection lines
    for(var gx = 0; gx < 20; gx++){
      ctx.strokeStyle = 'rgba(60,45,35,' + (0.06 + Math.sin(gx * 0.5) * 0.03) + ')';
      ctx.lineWidth = 0.5;
      ctx.beginPath();
      ctx.moveTo(W * 0.15 + gx * W * 0.038 + (mx - 0.5) * 30, H * 0.75);
      ctx.lineTo(W * 0.15 + gx * W * 0.038 + (mx - 0.5) * 60, H * 0.9);
      ctx.stroke();
    }
  }

  function drawCrystalHorse(){
    var cx = W * 0.5 + (mx - 0.5) * 30;
    var cy = H * 0.42 + (my - 0.5) * 20;
    var pulse = 1 + Math.sin(t * 0.02) * 0.08;

    // Multi-colored aura
    var ag = ctx.createRadialGradient(cx, cy, 15, cx, cy, 120 * pulse);
    ag.addColorStop(0, 'rgba(232,200,74,' + (0.25 * pulse) + ')');
    ag.addColorStop(0.3, 'rgba(200,150,60,' + (0.12 * pulse) + ')');
    ag.addColorStop(0.6, 'rgba(160,100,180,' + (0.04 * pulse) + ')');
    ...
    ctx.beginPath();
    ctx.moveTo(20, -4);
    ctx.quadraticCurveTo(32, -30, 26, -46);
    ctx.stroke();

    // Rose-gold highlight neck
    ctx.strokeStyle = 'rgba(200,160,200,0.5)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(18, -2);
    ctx.quadraticCurveTo(30, -28, 24, -44);
    ctx.stroke();

    // Copper highlight neck
    ctx.strokeStyle = 'rgba(200,140,100,0.5)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(22, -6);
    ctx.quadraticCurveTo(34, -32, 28, -48);
    ctx.stroke();
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(26, -48, 9, 5, -0.15, 0, Math.PI * 2);
    ctx.stroke();

    // Legs
    ctx.strokeStyle = 'rgba(255,220,140,0.5)';
    ctx.lineWidth = 1.5;
    [-16, -4, 8, 18].forEach(function(lx){
      ctx.beginPath();
      ctx.moveTo(lx, 12);
      ctx.lineTo(lx - 1, 32);
      ctx.stroke();
    ...
    // Crystalline mane
    ctx.strokeStyle = 'rgba(232,200,255,0.5)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(20, -8);
    ctx.quadraticCurveTo(24, -24, 28, -38);
    ctx.stroke();

    // Tail
    ctx.beginPath();
    ctx.moveTo(-28, 2);
    ctx.quadraticCurveTo(-38, -8, -34, -22);
    ctx.stroke();
  ...
  function drawOrbs(){
    orbs.forEach(function(o){
      o.baseAngle += o.speed;
      var p = o.radius / W;
      var ox = W * 0.5 + (mx - 0.5) * 60 * p + Math.cos(o.baseAngle) * o.radius;
      var oy = H * 0.45 + (my - 0.5) * 40 * p + Math.sin(o.baseAngle * 0.7) * o.radius * 0.5;
      var pu = 1 + Math.sin(t * 0.03 + o.pulse) * 0.25;

      var og = ctx.createRadialGradient(ox, oy, 0, ox, oy, o.size * 4 * pu);
      og.addColorStop(0, 'hsla(' + o.hue + ',80%,70%,' + (0.6 * pu) + ')');
      og.addColorStop(0.5, 'hsla(' + o.hue + ',60%,50%,' + (0.15 * pu) + ')');
      og.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = og;
      ctx.beginPath();
      ctx.arc(ox, oy, o.size * 4 * pu, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = 'hsla(' + o.hue + ',80%,75%,' + (0.8 * pu) + ')';
      ctx.shadowColor = 'hsla(' + o.hue + ',80%,70%,0.6)';
      ctx.shadowBlur = o.size * 3;
      ctx.beginPath();
      ctx.arc(ox, oy, o.size * pu, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    });
  }
  ...
  function drawParticles(){
    particles.forEach(function(p){
      var px = p.x * W;
      var py = p.y * H;
      var a = p.opacity * (0.4 + 0.6 * Math.sin(t * 0.03 + p.phase));
      ctx.fillStyle = 'hsla(' + p.hue + ',60%,70%,' + a + ')';
      ctx.shadowColor = 'hsla(' + p.hue + ',60%,70%,' + (a * 0.5) + ')';
  ...
  function animate(){
    t++;
    particles.forEach(function(p){
      p.y -= p.speed * p.z * 0.05;
      p.x += Math.sin(t * 0.015 + p.phase) * p.drift * 0.02;
      if(p.y < -0.1){ p.y = 1.05; p.x = Math.random() * 1.3 - 0.15; }
    });
    orbs.forEach(function(o){
      o.radius = W * (0.18 + (orbs.indexOf(o) % 3) * 0.08);
    });
    render();
...
// ── LIBRARY FLOOR CANVAS ──
(function(){
  var canvas = document.getElementById('libraryFloorCanvas');
  if(!canvas) return;
  var ctx = canvas.getContext('2d');
  var W, H, t = 0;
  ...
  function render(){
    ctx.clearRect(0, 0, W, H);
    ctx.fillStyle = 'rgba(6,3,8,0.3)';
    ctx.fillRect(0, 0, W, H);

    for(var i = 0; i < 8; i++){
      var ly = H * (0.1 + i * 0.1);
      ctx.strokeStyle = 'rgba(60,45,35,' + (0.1 - i * 0.008) + ')';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, ly + Math.sin(t * 0.01 + i) * 5);
      ctx.lineTo(W, ly + Math.cos(t * 0.01 + i) * 5);
      ctx.stroke();
    }
    ...
    particles.forEach(function(p){
      var px = p.x * W;
      var py = p.y * H;
      var a = p.opacity * (0.4 + 0.6 * Math.sin(t * 0.02 + p.phase));
  ...
  function animate(){
    ...
    particles.forEach(function(p){
      p.y -= p.speed * 0.04;
      if(p.y < -0.1){ p.y = 1.3; p.x = Math.random() * 1.3 - 0.15; }
    });
    render();
    requestAnimationFrame(animate);
  }
...
/* ═══════════════════════════════════════════════════════════
   PART 2: ARTICLES 1-5 (FOUNDATIONAL)
   ═══════════════════════════════════════════════════════════ */

var articles = [

  /* ── ARTICLE 1: Horse Psychology ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>o feel what a horse feels is to first understand that you are noise. You arrive trailing the scent of purpose, your pockets full of plans, your mind full of what was and what will be. You walk with the heavy step of a predator, an animal that does not need to listen before it moves. To the horse, you are a storm on the horizon. They see the shape of you, but they feel the intention — the hurry, the jagged edges of a mind that never rests. And in your presence, they become what they have always been: a statue carved from a single question. Am I safe?</p><p>To connect with them is to decide to become silent. Not just in your voice, but in your soul. You must let your agenda fall to the dust like a heavy coat. You stand at the edge of their space and learn to simply be. You begin to breathe. Not the shallow, forgotten breaths of a busy life, but deep, slow tides of air that tell every listening cell in their body: you are not a threat. You are just a creature, breathing.</p><p>And that is when the world changes. You stop seeing with your eyes and start feeling with your skin. The wind on your cheek is the same wind in their mane. The earth under your feet is the same earth under theirs. You are no longer a man and a horse. You are two bodies in a field, sharing the same small slice of the world.</p>' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>his is when the mirror appears. The horse begins to show you to yourself. If you carry tension — a tight jaw, a knot of worry in your gut — they will hold their own body tight, their head high, their muscles coiled. They will not come near the storm inside you. If you carry a grief you have refused to face, they will grow still and soft, their dark eye becoming a pool of sorrow that reflects your own. They absorb your truth and show it back without a word of judgment.</p><p>They have lived in fear. They understand the hum of hypervigilance — the way a body becomes a prison of held-back energy. And when they see it in you, they do not run. They recognise it. It is a language they were born speaking.</p><p>The moment of connection is not a grand event. It is a surrender. It is the slow exhale you have been holding. It is the quiet step they take toward you — not because you asked, but because you finally became a place of peace. The soft muzzle that touches your arm is a question asked without sound. In that touch, a universe of trust is exchanged. You are safe. And so are they.</p><p>To walk away from that is to feel the noise of the world rush back in. But you are different now. You carry the stillness of the field inside you. You have learned that the deepest communication requires no words at all — only a shared and silent breath.</p>' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">E</span>very decision a horse makes is filtered through sixty million years of prey animal evolution. To understand a horse is not simply to understand an animal. It is to understand a completely different kind of mind — one built for a world where a single mistake means death, and where reading the world fast is the only thing that matters.</p><p>Horses think in pictures. They think in feelings. They think in the language of energy and intention — so precise, so honest, that it makes human communication look clumsy by comparison. When a horse looks at you, it is not seeing a person. It is reading a pattern of energy. Your tension, your breath, the quality of your stillness, the direction of your gaze. Before you have spoken a word, the horse has already decided what you are.</p><p>This is not mysticism. This is biology. A prey animal that could not read the world fast and accurately did not live long enough to become an ancestor. Every horse alive today is the child of the most perceptive, the most sensitive, the most finely tuned readers that ever lived. To work with horses well, you must first accept this: they are not failing to understand you. You are failing to understand them. The horse is reading everything correctly. The question is whether you are giving it anything honest to read.</p>' },
      { type: 'section', heading: 'How Horses Think', sub: 'They do not think in words. They think in the truth of this moment.' },
      { type: 'text', section: 'How Horses Think', html: '<p><span class="rd-dropcap">H</span>orses do not think in sentences. They do not sort their experience into reasons or explanations. They think in pictures, in feelings, in what is happening right now. A horse scared once by a white plastic bag does not remember that plastic bags are scary. It remembers the exact feeling of that moment — the sudden movement, the crinkle of sound, the rush of fear, the need to run. When it sees another plastic bag, it does not think. It simply feels the same feeling, and the body moves.</p><p>This is not stupidity. This is intelligence so sharp, so finely made, that it has kept horses alive for millions of years. A prey animal that had to stop and think to figure out whether the sound in the bushes was dangerous or safe did not survive long enough to have young. The horses that remain are the children of those who felt danger and moved before the thinking mind could catch up.</p><p>You cannot explain to a horse why something is safe. You cannot tell it the needle will help, that the trailer leads somewhere good. The horse reads the world through different eyes entirely. It reads energy. It reads tension. It reads the tiny signals in your body that you do not even know you are sending. Before you move, the horse has already felt your intention to move.</p><p>Your thoughts are not private. Your fear, your anger, your rush, your doubt — all of it shows in your body before you have even noticed it yourself. The horse that seems difficult is often simply giving back exactly what you are giving. The rider who learns to become truly calm — not acting calm, but being calm — finds that the horse changes in that same moment.</p>' },
      ...
      { type: 'text', section: 'Equine Mindset', html: '<p><span class="rd-dropcap">A</span> horse lives in the present moment with a fullness that most humans will never know. It is not worrying about tomorrow. It is not replaying yesterday. It is here — now — fully alive in this breath, this step, this meeting. This is not wisdom. This is simply what it means to be a horse.</p><p>The equine mindset is built on a few basic truths. Safety comes first. Connection to the herd matters deeply. Movement is life. Trust must be earned. And the quality of your presence matters far more than any word you speak.</p><p>When you walk toward a horse, it is reading you before you are ten feet away. Are you calm or worried? Sure or unsure? Present or somewhere else entirely? The horse does not care what you say. It cares what you are. And if what you are is scattered, tight, or disconnected, the horse will show that back to you with perfect honesty. This is why horsemanship is as much about knowing yourself as it is about knowing horses. You cannot hide from a horse. You cannot fake being present. The horse knows. It always knows.</p><p>Horses do not hold grudges. They do not replay past hurts. If you make a mistake, if you lose your temper, if you handle something badly — the horse will give you another chance. It will meet you fresh in the next moment, willing to try again, as long as you are willing to be honest. This is the gift horses offer. Their complete, uncompromising honesty. They show us who we are. They teach us that the quality of the connection depends entirely on the quality of what we bring. And in teaching us that, they teach us something far more valuable than horsemanship. They teach us how to be human.</p>' },
  ...
  /* ── ARTICLE 2: The Flight Response ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>here is a sound that every horse alive knows before it is born. Not heard through the ears, but felt in the blood — a sound older than any language, older than the first human hand that ever reached toward a horse\'s neck. It is the sound of something moving in the grass when the grass should be still. It is the sound of threat. And the body of the horse — every muscle, every nerve, every breath — has been shaped by millions of years to answer that sound with one single, absolute response: run.</p><p>This is not a choice the horse makes. It is not a decision that passes through reason or reflection. By the time the thinking part of the brain has processed what the eyes have seen, the body is already moving. The legs are already carrying the horse away from what frightened it. This is not a flaw in the horse\'s design. This is the design working exactly as it was built to work.</p><p>And yet, every day, humans stand beside horses and feel frustration at this response. They pull harder on the rope. They raise their voice. They push the horse toward the thing it fears, convinced that force is the answer. They have mistaken the most sophisticated survival system ever built by nature for a problem to be corrected. It is not a problem. It is the horse. And until you understand it, you will never truly reach the animal standing in front of you.</p>' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">F</span>ear is not a character flaw in a horse. It is the most finely built survival system ever created by sixty million years of nature. When a horse spooks at a plastic bag, it is not being stupid. It is being exactly what it was made to be — a creature whose life depends on treating the unfamiliar as dangerous until proven otherwise.</p><p>The flight response is not a behavior problem. It is a nervous system working perfectly. The question for the horseman is never how to remove that response. You cannot, and you should not try. The question is how to become the thing the horse trusts more than it fears the unknown.</p><p>When you understand the biology — how the fear centre fires before the thinking brain has time to process, how stress chemicals flood the body in milliseconds, how the entire horse prepares for escape in the time it takes you to blink — you stop being frustrated by fear. You start having real compassion for what the horse is living through. And compassion, not correction, is where genuine horsemanship begins.</p>' },
      ...
      { type: 'text', section: 'The Brain That Runs Before It Thinks', html: '<p><span class="rd-dropcap">D</span>eep inside the horse\'s brain, buried beneath layers of more recent evolution, sits a small almond-shaped structure called the amygdala. It is the oldest part of the emotional brain. It does not think. It does not weigh consequences. It does not consider whether the white plastic bag by the arena fence has ever hurt a horse before. It simply receives information from the senses and, in the span of twelve milliseconds, fires a signal that floods the entire body with adrenaline and cortisol.</p><p>Twelve milliseconds. That is faster than the blink of a human eye. That is faster than the conscious mind can process a single thought. By the time the horse\'s rational brain — the part that, with training, can learn that plastic bags are safe — has even begun to process the visual information, the body is already in a full physiological state of emergency.</p><p>Heart rate doubles. Sometimes triples. Blood is diverted away from the digestive system and toward the large muscle groups of the legs. The pupils dilate. The nostrils flare. The entire body becomes, in an instant, a machine optimised for one single purpose: escape. This is called the fight-or-flight response, and in the horse it is almost exclusively flight. Unlike a predator, which may stand its ground when cornered, the horse\'s first, second, and third instinct is always to put distance between itself and the threat. Speed is safety. Distance is survival. This is sixty million years of successful living encoded into every cell of the animal\'s body.</p><p>When you understand this biology — truly understand it, not just intellectually but with genuine empathy — you stop asking why the horse is behaving this way. You start asking something far more useful: what can I offer this horse that its nervous system will accept as safe?</p>' },
      ...
      { type: 'text', section: 'How Horses Remember Fear', html: '<p><span class="rd-dropcap">F</span>ear memory in horses is not stored the way human memories are stored. Humans encode memories with context — they remember not just what happened, but where, when, with whom, and how they felt about it afterward. Time passes and the memory softens. Details blur. The emotional charge of a frightening event gradually diminishes as the brain files it away and moves forward.</p><p>This is not how the horse\'s brain works. Fear memories in horses are stored with extraordinary precision in the amygdala — and they are stored for life. A horse that was badly frightened in a horse trailer at age three will remember that fear at age twenty. Not as a story it tells itself, but as a full-body sensory experience that is re-triggered the moment the right stimulus appears. The smell of diesel fumes. The sound of a metal ramp. The feeling of reduced light and enclosed space. Any one of these can unlock the entire original terror in an instant.</p><p>This is why punishment is so damaging in horse training. When a human punishes a horse for displaying fear — hitting it, shouting at it, forcing it toward the thing that frightens it — they are not teaching the horse that the thing is safe. They are adding a second layer of threat to an already frightened nervous system. The horse now has two things to fear: the original stimulus and the human beside it. The fear becomes deeper, more complex, and far harder to resolve.</p><p>But the same principle that makes fear memory so persistent can also be used to create safety memory. Every time a horse is allowed to approach something frightening at its own pace, in its own time, and is not punished for hesitating — every time it discovers for itself that the scary thing did not harm it — a new memory is laid down alongside the old one. Not replacing it, but offering an alternative. Over time, with patience and consistency, the safety memory can become stronger than the fear memory. The horse learns, not because it was told, but because it was given the space to discover.</p><p>This process cannot be rushed. It cannot be forced. It can only be offered, again and again, until the horse chooses to trust what the evidence is showing it. That is desensitisation done honestly — not flooding the horse with the frightening thing until it gives up resisting, but walking beside it, slowly, as it learns that the world is safer than its instincts have always told it.</p>' },
      ...
      { type: 'text', section: 'Safety in Numbers', html: '<p><span class="rd-dropcap">I</span>n the wild, a horse alone is a horse in danger. Not because solitude itself is dangerous, but because every second a horse is without companions, it is bearing the full weight of vigilance alone. There are no other eyes watching the horizon. No other ears turning toward that sound in the distance. No other nose catching a scent on the wind. The solitary horse must be alert to everything, always, without rest.</p><p>This is why isolation is one of the most psychologically damaging things that can be done to a horse. Stabled horses kept without visual or physical contact with other horses show measurable increases in stress hormones, stereotypic behaviors — weaving, crib-biting, box walking — and heightened reactivity. They are not being difficult. They are expressing the profound biological distress of an animal whose survival system is telling it that it is in danger.</p><p>Within a herd, the vigilance is shared. While one horse grazes with its head down, others maintain watch. The pattern rotates naturally, without instruction or command. It is a distributed safety system, each horse contributing its senses to a collective awareness that no single animal could maintain alone.</p><p>This herd instinct is the key to understanding why horses are so profoundly affected by the emotional state of the humans around them. In the absence of horse companions, many horses will transfer their herd bonding to humans, other species, or even objects. They are not being foolish or sentimental. They are doing what their biology requires — finding something to be part of, something to trust, something that makes standing still in the world feel survivable.</p><p>When you earn the trust of a horse, you are not merely earning its affection. You are being accepted into its safety system. It is allowing you to be the one it turns to when something frightens it — the one whose stillness or calm it uses to calibrate its own response. That is not a small thing. For a prey animal, allowing another creature into its circle of trust is the most significant decision it can make. It is staking its survival on you.</p>' },
  ...
  /* ── ARTICLE 3: Trust vs Control ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>here is a question every horseman must eventually answer — not with words, but with choices made in the arena every single day. Do you want to control your horse, or do you want to be trusted by it? These are not the same goal. They do not produce the same horse. And they do not ask the same things of you as a person.</p><p>Control gives you a horse that obeys when it cannot escape. Trust gives you a horse that chooses to be with you when all the exits are open. I have spent eleven years learning the difference. Only one of these is real horsemanship. The other looks like it from the outside, but has a hollow sound when you knock on it.</p><p>The horse that is controlled knows where the boundary is. The horse that trusts you has no desire to find the boundary, because it has no desire to leave. One is managing its captivity. The other has chosen its companionship. And the gap between those two things is as wide as the sky.</p>' },
      ...
      { type: 'text', section: 'The Partnership That Changes Everything', html: '<p><span class="rd-dropcap">I</span> remember the first time a horse walked across an empty field to meet me not because it was feeding time, not because it had been trained to, but simply because I had been sitting quietly at the fence long enough that my presence had become something worth coming toward. It walked across that field slowly, head low, and stood beside me without any contact, without any request. It simply stood there. And in that standing, I understood for the first time what all of this is actually for.</p><p>Partnership is the word people use, but it is often misunderstood. Partnership does not mean the horse always does what you want. It means the horse participates with you — genuinely, willingly, with something behind its eyes that is more than compliance. You feel it in the rhythm of movement when it gives you its back without resistance. You feel it when it seeks you out in the field. You feel it in the rare, perfect moments when you ask for something and the horse gives it before the ask is finished — not because it anticipated the signal, but because it was already thinking the same thought.</p><p>Getting there takes time. More time than control does. A horse trained through pressure alone can be rideable in weeks. A horse that genuinely trusts a human takes months, sometimes years, depending on what the horse has lived through. But the result is not comparable. The controlled horse performs. The trusting horse participates. And in the difference between those two things is the whole reason why some people spend their entire lives in pursuit of what horses can offer, and never want to stop.</p>' },
  ...
  /* ── ARTICLE 4: Reading Body Language ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">H</span>orses speak in the language of the body — constantly, precisely, and honestly. They do not have the human ability to say one thing while meaning another. Every ear position, every shift of muscle, every change of weight, every flick of the tail is a sentence in a language that has no room for deception. Learning to read it is the single most important skill a horseman can develop. Everything else — every technique, every method, every tool — is secondary to this.</p><p>I spent my first year with horses watching their behavior and seeing nothing meaningful. I thought a horse pinning its ears meant it was bad-tempered. I thought a horse turning away meant it was disinterested. I was reading the words but not understanding the language. It was only when I began to slow down — truly slow down, and watch without an agenda — that the sentences began to make sense. Once they did, everything changed. The horse stopped being an animal I managed. It became a conversation I was part of.</p><p>This is what I want to give you. Not a checklist. Not a diagram. But a way of seeing that, once learned, cannot be unlearned. The horse is always talking. The only question is whether you are listening.</p>' },
      { type: 'section', heading: 'The Ears', sub: 'The most readable part of a horse. Every position is a sentence.' },
      { type: 'text', section: 'The Ears', html: '<p><span class="rd-dropcap">T</span>he ears are the horse\'s most expressive feature, and the first place a horseman looks when trying to understand what is happening inside the horse\'s mind. They rotate independently, they respond instantly to both external stimuli and internal emotional state, and they are almost impossible for the horse to consciously control — which makes them one of the most honest sources of information the horse has.</p><p>Ears forward means the horse is interested in something. Focused. Alert. Both ears locked forward onto a single point means the horse has found something that concerns it — it is gathering information and deciding what to do next. This is the position that precedes a spook. Do not push forward here. Give the horse a moment to process.</p><p>Ears soft and relaxed to the side — sometimes called "lop-eared" — means a deeply calm horse. This is the position of true rest, of a horse that has let its guard down completely. If a horse you are working with drops into this position, stop what you are doing and let it stand. You have reached a place of real relaxation. Preserve it.</p><p>One ear forward, one ear back means the horse is splitting its attention between two things — the human in front of it and something behind it, or the task it is doing and something happening to its side. This is not distraction in the negative sense. It is the horse doing what its biology demands: staying aware of its entire environment simultaneously.</p><p>Ears pinned flat back is the signal that requires the most respect. This is not simply irritation. Pinned ears say: I am at the end of my tolerance. I have been communicating discomfort for some time and the message has not been received. This is a warning that the next communication may be physical — a bite, a kick, a sharp movement. A horse that pins its ears is not being aggressive without reason. It is being honest about where it is. The horseman who responds to pinned ears by pushing harder is the horseman who eventually gets hurt.</p>' },
      ...
      { type: 'text', section: 'Reading the Whole Horse', html: '<p><span class="rd-dropcap">T</span>he trap that beginners fall into — and sometimes experienced riders too — is reading individual signals in isolation. A single pinned ear might mean the horse is listening to something behind it. A single swish of the tail might be a fly. A moment of high head might be curiosity. It is only when you read the whole horse — all the signals together, in context, over time — that you begin to understand what is actually being communicated.</p><p>The horse that has both ears pinned, a hard eye, a tight jaw, a high head, and a clamped tail is a horse that is telling you something very clearly and has been telling you for some time. The horse that has one ear back, a soft eye, a relaxed jaw, a low head, and a swinging tail is a horse that is largely comfortable but has a part of its attention on something you may not be aware of. Context matters. History matters. The relationship you have built over time matters.</p><p>The goal is not to memorize a list of signals. The goal is to develop a way of seeing — a habit of reading the whole horse in every moment, not just when something goes wrong. The horseman who reads calmly also reads escalation before it becomes a problem. They catch the early whisper of discomfort before it becomes a shout. They adjust before adjustment is urgent. This is what makes a horseman safe. Not strength. Not dominance. Not years of forcing horses through their resistances. Simply the ability to read, and the willingness to listen to what is being said.</p>' },
  ...
  /* ── ARTICLE 5: Common Human Mistakes ── */
  {
    ...
    pages: [
      ...
      { type: 'text', section: 'Escalating Into the Problem', html: '<p><span class="rd-dropcap">T</span>he third mistake is the one that does the most lasting damage: escalating when the horse resists. The logic seems obvious — the horse is not doing what you are asking, so you ask harder, louder, with more pressure. And sometimes the horse does comply. But what has it learned? It has learned that the human beside it becomes more threatening when it hesitates. It has learned that uncertainty leads to increased pressure. It has learned to manage its fear of the human, not to trust the human.</p><p>When a horse resists, the first question should never be how do I make this clearer with more pressure. The first question should be: why is the horse not ready to do this? Is it unclear? Is it frightened? Is it in pain? Is the ask too big for where the horse is today? Most resistance dissolves completely when the cause is addressed. Most resistance that is met with escalating pressure becomes stronger resistance — or a horse that shuts down and complies from survival instinct rather than from genuine willingness.</p><p>The skill of breaking things into small enough steps — asking for ten percent rather than a hundred, rewarding the try rather than insisting on the finish — changes everything about how horses respond. A horse that never feels overwhelmed by the size of the ask rarely needs to resist. It is given enough space to try, enough reward to want to continue, and enough consistency to trust that the ask will not grow unreasonably before it has managed the current one.</p>' },
      ...
      { type: 'text', section: 'Punishing the Try', html: '<p><span class="rd-dropcap">T</span>he fourth and perhaps most heartbreaking mistake is failing to reward the try — the moment when the horse has understood something, made an effort, moved in the right direction — and instead continuing to push for more, immediately, without acknowledging what was just offered.</p><p>A horse that tries and is ignored, or tries and is immediately asked for more without a moment of recognition, learns that trying does not change anything. The pressure does not release with effort. It only releases when the perfect answer is delivered. For a horse, this is profoundly discouraging. The willingness to try — which is the most precious thing a horse can offer — begins to diminish. The horse becomes dull, mechanical, doing just enough to make the pressure stop. The joy goes out of it.</p><p>The try does not need to be the finished thing. It needs to be seen and valued. A slight softening in the jaw, a single step in the right direction, a moment of leaning toward the answer rather than away — all of these deserve acknowledgment. Release the pressure. Pause. Let the horse process. Then ask again. Build the try into the finish over many repetitions, over many sessions, with patience that communicates to the horse: I see you. I see your effort. That matters here. A horse that believes its try matters will keep trying. And a horse that keeps trying will eventually give you everything.</p>' },
...
/* ═══════════════════════════════════════════════════════════
   PART 3: ARTICLES 6-10 (PRACTICAL)
   Paste this directly after Part 2's closing "]"
   ═══════════════════════════════════════════════════════════ */
  ...
  /* ── ARTICLE 6: How to Build Trust ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>rust between a human and a horse is not a feeling. It is a behavior. And like all behavior, it is built through repetition — through a thousand small moments where you did what you said you would, where you did not punish what you could not understand, where you stayed when it would have been easier to leave.</p><p>The first step is deceptively simple: stop trying to earn trust and start being trustworthy. Those are not the same thing. Trying to earn trust often looks like pressure — pushing the horse to respond, to come closer, to accept what it is not yet ready to accept, because you want to see the evidence of the relationship today. Being trustworthy requires only one thing: consistency. The horse that sees the same person every single time, no matter how the session is going, learns that this person is safe. And safety is the only foundation on which anything else can be built.</p><p>Trust is built in layers. The first layer is physical safety — the horse learning that you will not hurt it. The second is emotional safety — the horse learning that you will not overwhelm it. The third is relational safety — the horse learning that what happens between you is fair, and that its communication will be heard. Each layer takes time. Each layer must be genuinely established before the next one is possible. You cannot rush this. The horse knows when you are trying to shortcut it. And every shortcut costs more time than it saves.</p>' },
      ...
      { type: 'text', section: 'Step One: Be Worth Approaching', html: '<p><span class="rd-dropcap">T</span>he first concrete step is learning to stand in a way that is genuinely inviting rather than merely nonthreatening. These are different. A human standing perfectly still can still radiate intensity, urgency, or desire — all of which a horse reads as pressure. The horse you are trying to connect with feels your wanting it as a form of demand. It is not wrong to feel that. It simply is not helping.</p><p>Stand sideways rather than facing the horse directly. Drop your eyes slightly — not all the way to the ground, but breaking the forward-focused gaze that predators use. Breathe slowly. Deliberately. Let your weight settle into your feet. Exhale, and let your shoulders drop with the exhale. You are not performing relaxation. You are practising it, genuinely, in your own body first.</p><p>Do nothing. Ask for nothing. Want nothing for this moment. Simply be a quiet presence in the horse\'s space. Wait. The horse will eventually turn to look at you properly, will consider you, will — if you have been consistent enough over enough sessions — decide to walk toward you. When it does, let it come. Do not lean forward to meet it. Let it close the last distance itself. The horse that walks across the field to you has made a choice. That choice is the first real brick in the wall of trust.</p>' },
      ...
      { type: 'text', section: 'Step Two: Make Every Interaction Honest', html: '<p><span class="rd-dropcap">O</span>nce the horse is approaching willingly, the work becomes about the quality of what happens when it is near you. Every interaction either adds to the trust account or withdraws from it. The goal is to make the vast majority of interactions deposits — moments where the horse discovers that being with you leads to comfort, to release, to genuine ease.</p><p>When you handle the horse, be clear in what you are doing. Do not fumble with equipment in ways that create sudden unpredictable movements. Do not grab. Move slowly through sensitive areas — ears, legs, belly — and watch for the horse\'s response. If it braces, pause. Wait for it to release before continuing. You are teaching it that it does not need to brace against you, because you will always wait for the release. This takes longer than just pushing through. It also teaches the horse something that no amount of forcing will ever teach: that its communication changes what happens.</p><p>End every session well. Not every session will go perfectly — some days the horse is unsettled, some days you are not at your best. But make the effort to end on a note of softness. A moment of stillness. A breath together. The last thing that happens in a session is what the horse carries into the next one. If the last thing is pressure and frustration, that is what greets you at the gate the next morning. If the last thing is release and quiet, that is what the horse remembers you by.</p>' },
  ...
  /* ── ARTICLE 7: Why Your Horse Does Not Listen ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">W</span>hen a horse stops listening, the natural human response is to push harder, ask louder, repeat with more force. This is the most natural thing in the world to do. And it is almost always wrong.</p><p>A horse that is not listening is not defying you. It is telling you something. It is saying: I do not understand. Or: I am not ready. Or: I am in pain. Or, most often: I do not trust that what happens next will be safe for me. These are not acts of stubbornness. They are honest messages from an animal that cannot use words.</p><p>In eleven years with horses, I have found three real reasons why horses stop responding. The first is clarity — the horse does not understand what is being asked. The second is physical — there is pain or discomfort. The third is relational — there is not enough trust to try something uncertain. When you stop asking how do I make the horse listen, and start asking which of these three things is happening right now, you stop fighting and start solving. And when you solve the real problem, the listening always returns.</p>' },
      ...
      { type: 'text', section: 'The Clarity Problem', html: '<p><span class="rd-dropcap">T</span>he most common reason a horse does not respond is simply that it does not understand what is being asked. This seems obvious. Yet most people, when their horse does not respond to a signal, repeat the exact same signal with more force. They have not asked themselves the honest question: have I actually taught this clearly?</p><p>Horses learn through the release of pressure, not through the application of it. When you apply a signal and the horse offers any movement — any step, any shift, any softening — toward the answer you want, releasing the pressure in that moment teaches the horse that the movement was correct. If you continue the pressure through the movement, the horse learns nothing except that pressure is continuous. It does not understand what made the pressure stop because the pressure did not stop at the right moment.</p><p>Before deciding a horse is being unresponsive, ask yourself: have I broken this request down into small enough steps? Have I released clearly at exactly the right moment? Have I repeated this enough times, with enough consistency, for the horse to have learned the pattern? Often the answer to at least one of these questions is no. Go back to the beginning. Ask smaller. Release clearer. Be more consistent. The horse that seemed to be ignoring you will often surprise you with its attention and effort when the ask becomes genuinely understandable.</p>' },
      ...
      { type: 'text', section: 'The Trust Problem', html: '<p><span class="rd-dropcap">T</span>he third reason horses stop listening is the one that requires the most honest self-reflection: there is not enough trust in the relationship for the horse to try something uncertain. This horse knows what happens when it tries and gets it wrong. It has learned, through experience, that mistakes bring pressure, confusion, or punishment. It is not being stubborn. It is being careful with the only body it has.</p><p>This horse needs less training and more relationship. It needs sessions that are not about asking for new things, but about confirming old ones — things it already knows how to do well, asked in a way that produces ease and reward rather than pressure and correction. It needs to rebuild its confidence in the act of trying, to rediscover that effort is safe, that mistakes do not cost much, that the human beside it is genuinely on its side.</p><p>This takes time, and it takes a willingness to set aside the training agenda for as long as the relationship needs. Some horses need weeks. Some need months. But the investment is always returned. The horse that rebuilds its trust in you does not just begin listening again — it begins listening in a way it never has before. With genuine willingness. With something in its eye that was not there before. That is not just a better-trained horse. That is a different kind of partnership entirely.</p>' },
  ...
  /* ── ARTICLE 9: First Time Handling ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>he first impression you make on a horse is written in its memory with a permanence no later session can fully erase. There is no second first meeting. Everything you bring — your speed, your energy, the tension in your hands, the quality of your breath — the horse will carry forward into every interaction that follows.</p><p>This is not meant to frighten you. It is meant to give you the most useful gift I know: the understanding that the quality of the first meeting matters enormously, and that getting it right is not about having skill or experience. It is about slowing down, being genuinely present, and bringing a quality of respect to the encounter that the horse can feel.</p><p>I have watched people with thirty years of experience make terrible first introductions, and I have watched complete beginners, properly prepared and genuinely quiet in themselves, have extraordinary first meetings with horses. The difference was never about experience. It was always about presence. About the person leaving their hurry and their plans outside the gate, and walking in as simply a human being, curious and calm, with nothing to prove.</p>' },
      ...
      { type: 'text', section: 'Safety Fundamentals', html: '<p><span class="rd-dropcap">S</span>afety around horses is not a list of rules to memorize. It is a way of moving and thinking that comes from understanding the horse\'s nature. When you understand why horses react the way they do, the "rules" become intuitive rather than imposed.</p><p>Stay out of the blind spots. Directly in front of the nose and directly behind the hindquarters are areas the horse cannot see. A touch arriving from a blind spot is, to the horse\'s nervous system, a touch arriving from nowhere — and the body responds with the violence of a startled prey animal before the mind has any say in the matter. Always let the horse know where you are. Touch starts on the neck or shoulder, where the horse can see you, and moves from there. If you must move behind the horse, stay very close to the body — within arm\'s reach of the hindquarters — so that any kick loses most of its force before it reaches you. Or stay far enough back that you are out of range entirely.</p><p>Move predictably. Do not make sudden movements. Do not raise your arms quickly. Do not drop things near the horse without warning. The horse\'s fear response fires faster than thought — give it no reason to fire. When you need to do something that might startle, narrate it first with your energy — slow down even more, breathe deliberately, let your body tell the horse before your hands do that something is about to happen.</p><p>Never wrap a lead rope around your hand or wrist. If the horse spooks and moves sharply, a rope around your hand can break bones or drag you off your feet. Hold the rope in loose coils. Learn to let go before the horse pulls you. The horse will not go far. Your hand and your relationship are more important than a brief chase around the yard.</p>' },
      ...
      { type: 'text', section: 'The First Touch', html: '<p><span class="rd-dropcap">T</span>he first time your hand touches a horse, it is a statement about everything that will follow. A hand that is tense communicates tension. A hand that grabs communicates urgency. A hand that is heavy and unresponsive communicates that it is not listening. A hand that is soft, warm, and genuinely alive — that is curious and present and responsive to what it finds — communicates something the horse has not felt from every human. It communicates that this person is paying attention.</p><p>Touch the neck first. The neck and shoulder are the areas where horses make physical contact with each other — mutual grooming, comfort, connection. They are the areas most associated with safety in the horse\'s experience. Begin there. A slow, confident stroke along the neck — not patting, which horses find less comfortable than humans imagine. A long, slow stroke, following the direction of the hair, with a pressure that is firm enough to be clear but not heavy enough to be a demand.</p><p>Watch the response. Does the horse soften toward the touch? Does the head lower slightly? Does a sigh escape? These are invitations to continue. Does the horse brace, move away, or tighten? These are communications that the touch is in the wrong place, at the wrong time, with the wrong quality. Listen to the response and adjust. The hand that listens to the horse\'s response is the hand the horse will learn to trust. And a horse that trusts your hands will eventually trust your direction. That is not a small thing. That is everything.</p>' },
  ...
  /* ── ARTICLE 10: Signs of Trust ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">A</span> horse\'s trust is not invisible. It is not a feeling you guess at or evaluate in hindsight. Trust, in a horse, is a behavior. And behavior is visible, readable, and honest. When you know what to look for, you will never again wonder whether what you have with your horse is real connection or simply a working arrangement that is holding together for now.</p><p>I want to be precise about this, because the equestrian world is full of people who believe they have their horse\'s trust when what they actually have is their horse\'s compliance. These are not the same thing. A horse that complies is a horse that has learned the cost of not complying. A horse that trusts is a horse that has chosen the relationship because it finds genuine safety, ease, and something like pleasure in it. The difference is visible in every session, if you know where to look.</p><p>The signs of real trust are both subtle and unmistakable. Once you have seen them — truly seen them, in a horse that genuinely trusts the person with it — you will recognize their absence everywhere else. And you will not be able to mistake the absence for presence ever again.</p>' },
      ...
      { type: 'text', section: 'Signs That Trust Is Missing', html: '<p><span class="rd-dropcap">T</span>he absence of trust shows just as clearly as its presence. The horse that moves away when you enter the field — consistently, not just today when something spooked it — is a horse that has not found your approach to be something worth staying for. The horse that is difficult to catch is often described as naughty or cunning. It is neither. It is a horse that has learned that being caught leads to experiences it would rather avoid.</p><p>The horse that is tense throughout a session — not because the work is difficult, but as a baseline state whenever you are present — is a horse whose nervous system has not found yours to be a source of regulation. It is still doing all the work of managing its own anxiety alone, without being able to use your calm as a reference point. This horse is not bad. It is isolated inside its own vigilance, doing its best to stay safe in a situation it does not feel genuinely safe in.</p><p>The horse that is compliant but disconnected deserves particular attention. It does what is asked. It does not resist. But there is nothing in its eye — it is somewhere else entirely, going through the motions. This horse has learned that cooperation is the path of least resistance. It has not learned that cooperation leads to genuine ease and reward. This is not a trustworthy partnership. It is a managed arrangement. And while it may look successful from the outside, it is not what horses are capable of offering when they are truly safe.</p><p>None of these signs are verdicts. They are information. They tell you where the relationship currently is, not where it has to stay. Every horse that shows these signs of missing trust has the capacity for real trust — it has simply not yet been given enough of the right experiences to build it. That is fixable. It requires patience, consistency, and honesty. But it is always fixable.</p>' },
...
/* ═══════════════════════════════════════════════════════════
   PART 4: ARTICLES 11-15 (UNIQUE EDGE) + READER ENGINE + BOOKS
   Paste this directly after Part 3's closing "]"
   ═══════════════════════════════════════════════════════════ */
  ...
  /* ── ARTICLE 11: What Horses Teach Us ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">A</span> horse has no ego. It has no agenda beyond what is real right now. It cannot pretend. It cannot perform. It can only respond to what is genuinely present. Spend enough time with horses and you will learn more about your real self — not the one you show the world — than any other method of self-understanding can offer.</p><p>What horses teach us above all is that our inner state is not private. We walk through the world believing we can perform competence while feeling inadequate, project calm while being anxious, present patience while carrying resentment. Horses expose this belief as the comfortable fiction it has always been.</p><p>The horse does not see the performance. It sees what is underneath. It reads the stress in your body, the tiny tensions in your muscles, the quality of your breath. Everything you have learned to hide from other humans, the horse has already read. And then it shows you — honestly, without cruelty, without judgment — exactly who walked in. This is a service of extraordinary value, if you have the courage to receive it.</p>' },
      ...
      { type: 'text', section: 'The Mirror You Cannot Lie To', html: '<p><span class="rd-dropcap">I</span> have watched people have revelations beside horses that they had been avoiding for years in other contexts. The person who insists they are patient until the horse reveals, without any accusation, that they are not. The person who believes they are calm until the horse shows them exactly how much they are holding. The person who thinks they are present until the horse demonstrates, by its inability to settle, that they are anywhere but here.</p><p>These revelations are not comfortable. They are not meant to be. But they are profoundly useful. The horse cannot be convinced to see what is not there. It cannot be charmed or reasoned with or persuaded to overlook what it is reading. It simply responds to what is actually present. And this makes it the most honest feedback mechanism most of us will ever encounter.</p><p>The people who grow most from their time with horses are the ones who learn to ask: what is the horse showing me about myself right now? Not what is wrong with the horse. What is the horse revealing about what I am bringing. This shift — from judgment of the horse to curiosity about oneself — is the moment horsemanship becomes something larger than a skill. It becomes a genuine practice of self-knowledge.</p>' },
      ...
      { type: 'text', section: 'What Horses Teach About Relationship', html: '<p><span class="rd-dropcap">T</span>he deepest thing horses teach us is something about the nature of relationship itself. A relationship that is worth anything is not built on what one party can extract from the other. It is built on genuine attention, genuine care, and a willingness to be affected by the other\'s experience. Horses demand this not through words but through the simple fact of what they are: creatures of extraordinary sensitivity who respond with complete honesty to what they are given.</p><p>If you approach a horse with the goal of getting something from it — a perfect session, an obedient horse, a result that proves your skill — the horse feels the agenda. It may comply. But it will not open. It will not offer the additional quality that genuine trust makes possible. The best horsemanship, in my experience, comes from people who have learned to enter the relationship without an agenda — to be curious about the horse, genuinely interested in its inner life, willing to be surprised and delighted and sometimes humbled by what they find.</p><p>This is not a skill you can practise only with horses. It is a way of being in relationships. A quality of attention and generosity and genuine curiosity about the other\'s experience. Horses teach it to us because they make it impossible to fake. But what they are teaching is not horsemanship. It is something larger. It is how to be genuinely present with another living being. And that, practised with a horse for long enough, tends to change the way you are present with every living being in your life.</p>' },
  ...
  /* ── ARTICLE 12: Presence and Intention ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">P</span>resence is not a concept. It is not a state you can think your way into. It is a physical reality — the quality of being fully here, fully alive, fully available to what is happening in this exact moment. Horses live in presence permanently. It is not something they achieve. It is simply what they are.</p><p>Most humans have lost the ability to do this. We are almost never fully here. We are partly in the meeting we just left, partly in the obligation ahead, partly in a conversation replaying itself on the inside of our skull. We bring all of this into the arena. The horse feels every bit of it. Not as metaphor. As real, physical, readable information.</p><p>To understand what a horse feels in your presence, try this: stand still beside it, close your eyes, and feel your own body. Feel your breath, your heartbeat, the tensions in your face, your jaw, your hands. Feel where you are holding. Now ask yourself honestly — if your survival depended on reading energy accurately, what would you make of the creature standing beside you?</p>' },
  ...
  /* ── ARTICLE 14: Silence, Energy, Movement ── */
  {
    ...
    pages: [
      ...
      { type: 'text', section: 'The Language of Energy', html: '<p><span class="rd-dropcap">E</span>nergy, in the context of horsemanship, is simply the quality of aliveness in a body. It is the difference between a body that is switched on — present, attentive, directed — and a body that is passive, unfocused, elsewhere. Both states communicate clearly to the horse. High, directed energy says: something is about to happen. Pay attention. Low, soft energy says: rest. Nothing is required. These are the two most basic messages in the horse\'s language, and every communication between horse and human is built from some combination of them.</p><p>Learning to use your energy deliberately is one of the most transformative skills in horsemanship. The horseman who can raise their energy — not by moving faster, but by becoming more internally directed, more alive, more focused — and feel the horse respond to that inner shift before any external aid has been given, has understood something essential. And the horseman who can lower their energy, genuinely and completely, and use it to bring a tense horse down rather than continuing to escalate, has understood something even more essential.</p><p>Energy is also the first thing horses read about your emotional state. Anxiety produces a particular quality of energy — scattered, high-frequency, alert. Calm produces a different quality — slow, deep, continuous. Horses categorise these patterns instantly, and they respond to them as information about whether the environment is safe. You cannot fake the energy of genuine calm. But you can practise it. And the more you practise it in the company of horses — who give you immediate, honest feedback — the more available it becomes in other areas of your life as well.</p>' },
  ...
  /* ── ARTICLE 15: Philosophy of Working With Horses ── */
  {
    ...
    pages: [
      { type: 'title' },
      { type: 'text', section: 'Opening', html: '<p><span class="rd-dropcap">T</span>here are two ways to work with a horse. They look similar from the outside — a horse and rider moving together with apparent ease and harmony. But from inside the horse, they feel completely different. And the horse always knows the difference, even when the audience does not.</p><p>The first way is the way of control and management. It is not necessarily cruel. It can be technically correct, professionally delivered, and produce impressive results. But its foundation is the human\'s agenda, the human\'s timeline. The horse learns to manage its life within the edges of what is permitted. It may be content. But it is not free.</p><p>The second way is the way of real partnership. It begins not with what you want the horse to do, but with understanding what the horse is. It asks for more curiosity than certainty, more listening than instruction. It produces a horse that is not performing for you — it is participating with you. Partnership requires two willing participants. Everything else, however graceful, is something less.</p>' },
      ...
      { type: 'text', section: 'The Life That Horses Offer', html: '<p><span class="rd-dropcap">I</span> did not come to horses because I wanted to ride. I came to them because I was, at a point in my life, looking for something more honest than most of what the human world offered. I found it. Not in a dramatic way. Not in a single moment of revelation. I found it in ten thousand mornings in a field, with an animal that showed me, patiently and without judgment, exactly who I was that day and exactly how much distance there was between who I was and who I wanted to become.</p><p>Horses have given me things that no other practice, no teacher, no book has given me. They have taught me to be still. They have taught me that my inner state is not private, and that learning to manage it is not weakness but one of the most important skills a person can develop. They have taught me that real communication requires real listening — not the polite performance of listening while you wait to speak, but genuine, open, willing attention to what another being is actually expressing. They have taught me that trust is built slowly, in the dark, through a thousand small acts of consistency, and that it is worth far more than anything that can be obtained quickly.</p><p>Most of all, they have taught me that the quality of a relationship — any relationship — depends entirely on the quality of what each party brings to it. You cannot take more than you give and call it a partnership. You cannot demand without offering. You cannot manage from a distance and expect genuine connection. These are lessons from the arena. They are also, I have come to believe, lessons for everything. And that, in the end, is why horses are not just animals I work with. They are some of the best teachers I have ever had.</p>' },
...
(function(){
  ...
  var artLabel = document.getElementById('artLabel');
  var artTitle = document.getElementById('artTitle');
  var artKeywords = document.getElementById('artKeywords');
  var artBody = document.getElementById('artBody');
  var artClose = document.getElementById('artClose');
  var openIdx = -1;

  function buildArticleHTML(art){
    var h = '';
    h += '<div style="text-align:center;padding:56px 24px 64px;border-bottom:.5px solid rgba(200,155,60,.1)">';
    h += '<div style="display:flex;align-items:center;gap:14px;max-width:280px;margin:0 auto 20px"><div style="flex:1;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.42))"></div><div style="width:7px;height:7px;background:#E8C84A;transform:rotate(45deg);box-shadow:0 0 10px rgba(200,155,60,.9)"></div><div style="flex:1;height:.5px;background:linear-gradient(90deg,rgba(200,155,60,.42),transparent)"></div></div>';
    h += '<span style="font-family:Cinzel,serif;font-size:.64rem;letter-spacing:6px;color:rgba(200,155,60,.35);text-transform:uppercase;display:block;margin-bottom:16px">' + art.label + '</span>';
    h += '<h3 style="font-family:Cinzel,serif;font-size:clamp(1.1rem,2.8vw,1.7rem);font-weight:600;color:#f0e0b8;letter-spacing:.5px;margin:0 auto 20px;max-width:600px;text-shadow:0 0 50px rgba(200,155,60,.18)">' + art.title + '</h3>';
    h += '<p style="font-family:EB Garamond,serif;font-style:italic;font-size:clamp(1rem,1.8vw,1.16rem);color:rgba(210,185,130,.4);max-width:440px;margin:0 auto 20px">' + art.tagline + '</p>';
    h += '<div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:center;margin-bottom:20px">';
    art.keywords.forEach(function(k){ h += '<span style="font-family:Cinzel,serif;font-size:.5rem;letter-spacing:1.8px;color:rgba(160,120,40,.5);text-transform:uppercase;padding:3px 9px;border:.5px solid rgba(180,130,40,.15)">' + k + '</span>'; });
    h += '</div><span style="font-family:Cinzel,serif;font-size:.52rem;letter-spacing:3px;color:rgba(200,155,60,.16);text-transform:uppercase">The Horseman\'s Journal · Praveen Kumar</span></div>';
    art.pages.forEach(function(pg){
      if(pg.type === 'title') return;
      if(pg.type === 'section'){
        h += '<div style="text-align:center;padding:68px 24px 60px;border-top:.5px solid rgba(200,155,60,.08);border-bottom:.5px solid rgba(200,155,60,.08)">';
        h += '<div style="display:flex;align-items:center;gap:14px;max-width:280px;margin:0 auto 14px"><div style="flex:1;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.36))"></div><div style="width:7px;height:7px;background:#E8C84A;transform:rotate(45deg);box-shadow:0 0 10px rgba(200,155,60,.6)"></div><div style="flex:1;height:.5px;background:linear-gradient(90deg,rgba(200,155,60,.36),transparent)"></div></div>';
        h += '<span style="font-family:Cinzel,serif;font-size:.58rem;letter-spacing:6px;color:rgba(200,155,60,.26);text-transform:uppercase">Section</span>';
        h += '<h4 style="font-family:Cinzel,serif;font-size:clamp(1.45rem,4vw,2.3rem);font-weight:600;color:#e8d5a0;letter-spacing:.8px;margin:14px 0 8px">' + pg.heading + '</h4>';
        h += '<div style="width:72px;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.36),transparent);margin:0 auto 14px"></div>';
        h += '<p style="font-family:EB Garamond,serif;font-style:italic;font-size:clamp(.98rem,1.7vw,1.14rem);color:rgba(200,170,100,.34)">' + pg.sub + '</p></div>';
      }
      if(pg.type === 'text'){
        h += '<div style="padding:52px 0 0"><div style="display:flex;align-items:center;gap:14px;margin-bottom:36px;padding-bottom:14px;border-bottom:.5px solid rgba(200,155,60,.1)"><span style="font-family:Cinzel,serif;font-size:.58rem;letter-spacing:4px;color:rgba(200,155,60,.3);text-transform:uppercase;white-space:nowrap">' + pg.section + '</span><div style="flex:1;height:.5px;background:rgba(200,155,60,.07)"></div></div><div style="font-family:EB Garamond,serif;font-size:clamp(1.12rem,1.9vw,1.34rem);line-height:2.15;color:rgba(232,212,178,.82)">' + pg.html + '</div></div>';
      }
      if(pg.type === 'quote'){
        h += '<div style="text-align:center;padding:60px 24px"><div style="width:100%;max-width:380px;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.26),transparent);margin:0 auto 16px"></div><div style="display:flex;align-items:center;gap:14px;max-width:280px;margin:0 auto 16px"><div style="flex:1;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.36))"></div><div style="width:7px;height:7px;background:#E8C84A;transform:rotate(45deg);box-shadow:0 0 10px rgba(200,155,60,.6)"></div><div style="flex:1;height:.5px;background:linear-gradient(90deg,rgba(200,155,60,.36),transparent)"></div></div><p style="font-family:EB Garamond,serif;font-style:italic;font-size:clamp(1.12rem,2.2vw,1.48rem);color:rgba(225,195,135,.68);line-height:1.85;max-width:560px;margin:0 auto">' + pg.text + '</p><div style="display:flex;align-items:center;gap:14px;max-width:280px;margin:16px auto 0"><div style="flex:1;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.36))"></div><div style="width:7px;height:7px;background:#E8C84A;transform:rotate(45deg);box-shadow:0 0 10px rgba(200,155,60,.6)"></div><div style="flex:1;height:.5px;background:linear-gradient(90deg,rgba(200,155,60,.36),transparent)"></div></div><div style="width:100%;max-width:380px;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.26),transparent);margin:16px auto 0"></div><span style="font-family:Cinzel,serif;font-size:.52rem;letter-spacing:3px;color:rgba(200,155,60,.2);text-transform:uppercase">Praveen Kumar · The Horseman\'s Journal</span></div>';
      }
    });
    h += '<div style="text-align:center;padding:52px 24px 60px;border-top:.5px solid rgba(200,155,60,.08)"><div style="display:flex;align-items:center;gap:14px;max-width:280px;margin:0 auto 20px"><div style="flex:1;height:.5px;background:linear-gradient(90deg,transparent,rgba(200,155,60,.36))"></div><div style="width:7px;height:7px;background:#E8C84A;transform:rotate(45deg);box-shadow:0 0 10px rgba(200,155,60,.6)"></div><div style="flex:1;height:.5px;background:linear-gradient(90deg,rgba(200,155,60,.36),transparent)"></div></div><span style="font-family:Cinzel,serif;font-size:.56rem;letter-spacing:5px;color:rgba(200,155,60,.2);text-transform:uppercase">End of Article</span></div>';
    return h;
  }

  function openArticle(i){
    var art = articles[i];
    artLabel.textContent = art.label;
    artTitle.textContent = art.title;
    artKeywords.innerHTML = '';
    art.keywords.forEach(function(k){
      var s = document.createElement('span'); s.className = 'art-kw'; s.textContent = k;
      artKeywords.appendChild(s);
    });
    artBody.innerHTML = buildArticleHTML(art);
    panel.classList.add('open');
    setTimeout(function(){ panel.scrollIntoView({behavior:'smooth',block:'start'}); }, 80);
    openIdx = i;
  }

  function closeArticle(){ panel.classList.remove('open'); openIdx = -1; }
  artClose.addEventListener('click', closeArticle);
  document.addEventListener('keydown', function(e){ if(e.key === 'Escape' && panel.classList.contains('open')) closeArticle(); });

  /* ═══════════════════════════════════════════
     BUILD BOOK GRID
     ═══════════════════════════════════════════ */
  var bookData = [
    {cat:'foundational',num:'01',img:'article1.jpg',bg:'#1a0d00,#0d0800',icon:'🐴',label:'Understanding the Horse Mind',title:'Horse Psychology: How Horses Think and Perceive Humans',text:'Every decision a horse makes is filtered through sixty million years of prey animal evolution.',kws:['horse psychology','how horses think','equine mindset']},
    {cat:'foundational',num:'02',img:'article2.jpg',bg:'#0a0e1a,#060b14',icon:'🐎',label:'The Primal Architecture',title:'Prey Animal Instincts in Horses: Why Fear Shapes Everything',text:'Fear is not a character flaw in a horse. It is the most sophisticated survival system ever built by nature.',kws:['prey animal instincts','flight response','horse fear triggers']},
    {cat:'foundational',num:'03',img:'article3.jpg',bg:'#0f0a00,#1a1000',icon:'🐴',label:'The Central Question',title:'Trust Versus Control in Horse Training: What Actually Works',text:'Control gives you a horse that obeys when it cannot escape. Trust gives you a horse that chooses to be with you.',kws:['trust vs control','natural horsemanship','horse training philosophy']},
    {cat:'foundational',num:'04',img:'article4.jpg',bg:'#080012,#050009',icon:'🐎',label:'The Silent Vocabulary',title:"How to Read a Horse's Body Language from Beginner to Advanced",text:'Horses speak in the language of the body, constantly, precisely, and honestly.',kws:['horse body language','horse ears meaning','signs of stress horses']},
    {cat:'foundational',num:'05',img:'article5.jpg',bg:'#120800,#1e0f00',icon:'🐴',label:'What We Get Wrong',title:'Common Human Mistakes with Horses and Why Horses React',text:'Most horses do not have problems. Most horses have people.',kws:['mistakes in horse training','why horses misbehave','training mistakes beginners']},
    {cat:'practical',num:'06',img:'article6.jpg',bg:'#001008,#000a05',icon:'🐎',label:'Step by Step',title:'How to Build Trust with a Horse: A Complete Step by Step Guide',text:'Trust is not built in a single day or a single session. It is built in a thousand small honest moments.',kws:['how to build trust','bonding with a horse','gain horse trust']},
    {cat:'practical',num:'07',img:'article7.jpg',bg:'#0d0015,#080010',icon:'🐴',label:'The Real Diagnosis',title:'Why Your Horse Does Not Listen to You: The Real Reasons Explained',text:'When a horse stops listening, most people think the horse has a problem.',kws:['horse not listening','horse training problems','why horse ignores commands']},
    {cat:'practical',num:'08',img:'article8.jpg',bg:'#001a10,#001208',icon:'🐎',label:'Finding Stillness',title:'How to Calm a Nervous or Anxious Horse',text:'You cannot calm a horse by fighting its nervous system. You can only offer it something steadier.',kws:['how to calm a horse','anxious horse behavior','nervous horse training']},
    {cat:'practical',num:'09',img:'article9.jpg',bg:'#1a0a00,#120700',icon:'🐴',label:'The First Meeting',title:'First Time Handling a Horse: What You Must Know',text:'The first impression you make on a horse is written in its memory with indelible ink.',kws:['first time horse tips','horse safety basics','beginner horse handling']},
    {cat:'practical',num:'10',img:'article10.jpg',bg:'#0a001a,#060012',icon:'🐎',label:'Reading the Bond',title:'Signs Your Horse Trusts You and Signs It Does Not',text:"A horse's trust is not a feeling. It is a behaviour. And like all behaviour, it is visible and honest.",kws:['signs a horse trusts you','horse bonding signs','horse trust behavior']},
    {cat:'edge',num:'11',img:'article11.jpg',bg:'#001014,#00080d',icon:'🐴',label:'The Mirror in the Arena',title:'What Horses Teach Us About Human Behavior',text:'A horse has no ego, no agenda, no ability to lie. It can only respond to what is real.',kws:['what horses teach humans','lessons from horses','horse psychology humans']},
    {cat:'edge',num:'12',img:'article12.jpg',bg:'#1a1000,#120a00',icon:'🐎',label:'Presence and Intention',title:'To Feel What a Horse Feels: Understanding Presence and Intention',text:'Presence is not a concept. It is a physical state — being fully here, fully alive.',kws:['horse sensitivity to humans','connection with horses','horse energy intention']},
    {cat:'edge',num:'13',img:'article13.jpg',bg:'#00100a,#000d07',icon:'🐴',label:'The Invisible Rein',title:'Why Horses React to Your Mind and Not Just Your Actions',text:'Before your hand moves, your mind has already moved. A horse reads the thought behind the action.',kws:['do horses sense emotions','horse awareness','horse reaction to humans']},
    {cat:'edge',num:'14',img:'article14.jpg',bg:'#0a0015,#060010',icon:'🐎',label:'The Ancient Language',title:'Silence, Energy, and Movement: The Language Horses Understand',text:'There is a language older than words. Horses speak it fluently.',kws:['horse communication','non verbal horses','how horses communicate']},
    {cat:'edge',num:'15',img:'article15.jpg',bg:'#1a0800,#120600',icon:'🐴',label:'The Deepest Truth',title:'The Philosophy of Working with Horses and Not Against Them',text:'There are two ways to work with a horse. One produces obedience. The other produces partnership.',kws:['natural horsemanship','ethical horse training','working with horses']}
  ];
  ...
  bookData.forEach(function(b, i){
    ...
    wrap.setAttribute('data-category', b.cat);
    wrap.setAttribute('data-article', i);
    var ribbonText = b.cat === 'foundational' ? 'Foundational' : b.cat === 'practical' ? 'Practical' : 'Unique Edge';
    wrap.innerHTML = '<div class="book-3d"><div class="book-spread"><div class="book-left"><img src="' + b.img + '" alt="' + b.title + '" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';"><div class="book-left-placeholder" style="display:none;background:linear-gradient(145deg,' + b.bg + ');">' + b.icon + '</div><div class="book-ribbon">' + ribbonText + '</div><div class="book-num">' + b.num + '</div></div><div class="book-spine"></div><div class="book-right"><div class="book-watermark">' + b.icon + '</div><div class="book-page-num">The Horseman\'s Journal | Volume ' + b.num + '</div><div class="book-art-label">' + b.label + '</div><div class="book-art-title">' + b.title + '</div><div class="book-rule"></div><p class="book-art-text">' + b.text + '</p><div class="book-keywords">' + b.kws.map(function(k){ return '<span class="book-kw">' + k + '</span>'; }).join('') + '</div><span class="book-read">Read Article</span></div></div><div class="book-edge"></div><div class="book-shadow"></div></div>';
    wrap.addEventListener('click', function(){ openArticle(i); });
    grid.appendChild(wrap);
  });

  /* ── FILTERS ── */
  document.querySelectorAll('.chr-filter-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      document.querySelectorAll('.chr-filter-btn').forEach(function(b){ b.classList.remove('active'); });
      btn.classList.add('active');
      var f = btn.dataset.filter;
      document.querySelectorAll('.book-wrap').forEach(function(bk){
  ...
  var bookObs = new IntersectionObserver(function(entries){
    entries.forEach(function(e, i){
      if(e.isIntersecting) setTimeout(function(){ e.target.classList.add('visible'); }, i * 100);
    });
  }, { threshold: 0.08 });
  document.querySelectorAll('.book-wrap').forEach(function(b){ bookObs.observe(b); });
